#!/usr/bin/env node
/** Google Indexing API — explicit URLs only (invest-gulf.com). */
...
if (!urls.length) {
  console.error('Usage: node scripts/submit-google-explicit.mjs https://invest-gulf.com/...');
  process.exit(1);
}
...
if (okUrls.length) {
  const r = recordSubmitted({ siteFolder: 'invest-gulf-website', urls: okUrls, channel: 'google' });
