...
function scoreProvenance(docId, index) {
  ...
  const re =
    /(?:(?:\b(?:AED|SAR|QAR|OMR|BHD|KWD|USD|GBP|EUR|SGD|INR)|\$)\s?\d[\d,]*(?:\.\d+)?(?:\s*(?:million|bn|k))?|\d+(?:\.\d+)?%|\d[\d,]*\s*(?:business\s+)?(?:days?|weeks?|months?|years?))/gi;
...
export function scoreDocument(docId, index, { requireRegistry = true } = {}) {
  ...
  // Counted rather than shared, so writing more prose around a pasted block
  // cannot dilute it. Two copies of a line is a coincidence worth forgiving;
  // the labelled sets put every honest file at exactly one.
  if (signals.repeatedLines.max > 2) {
    add((signals.repeatedLines.max - 2) * 4, 'pasted-block',
      `one line appears ${signals.repeatedLines.max} times in this article: "${signals.repeatedLines.worst.slice(0, 70)}"`);
  }
