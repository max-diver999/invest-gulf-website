...
const LEAD_FROM_EMAIL = import.meta.env.LEAD_FROM_EMAIL || `Invest Gulf <${SITE.email}>`;
...
async function sendAutoReply(to: string, name: string, context: string) {
  ...
  const topic = context ? escapeHtml(context) : 'your Gulf property enquiry';
    ...
    if (!isHealthcheck) {
      try {
        await sendLeadNotifyEmail({
          subject: 'New lead — invest-gulf.com',
          htmlBody: lines,
          from: LEAD_FROM_EMAIL,
        });
      } catch (err) {
