...
const LEAD_FROM_EMAIL =
  import.meta.env.LEAD_FROM_EMAIL || `Invest Gulf <${SITE.email}>`;
...
function normalizeSiteUrl(value: unknown): string {
  ...
  try {
    ...
    if (host !== 'invest-gulf.com' && !host.endsWith('.vercel.app')) {
      return '';
    }
    return url.href;
  } catch {
...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    const lines = [
      '💬 <b>WhatsApp click | invest-gulf.com</b>',
    ...
    const plainLines = [
      'WhatsApp click | invest-gulf.com',
      `Ref: ${refCode}`,
    ...
    try {
      await sendLeadNotifyEmail({
        subject: `WhatsApp click — invest-gulf.com (${refCode})`,
        htmlBody: plainLines,
