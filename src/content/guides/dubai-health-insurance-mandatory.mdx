...
Yes. Dubai Law No. 11 of 2013 requires every resident to hold valid DHA-approved cover, and employers must insure employees while sponsors must insure dependants they bring in.
...
A valid policy must be issued by a DHA-licensed insurer, meet Essential Benefits Plan minimums, and include a DHA policy reference you can show during visa processing.
...
EBP is the minimum package for workers earning under AED 4,000 per month and covers emergency care, GP visits, referrals, basic maternity, and generic drugs with low co-pays.
...
Enhanced plans raise annual benefit limits and hospital networks, while premium tiers add dental, maternity choice, and international second opinions.
...
Employers must enroll every employee on a DHA-approved plan, pay EBP premiums for eligible low-wage staff, and face AED 500 monthly fines per uninsured worker if they fail.
...
Self-sponsored residents should budget AED 3,000 to 18,000 per adult depending on network tier, with family policies often reaching AED 18,000 to 45,000 per year.
...
Yes. Sponsors must arrange DHA-compliant policies for maids, nannies, and drivers before visa stamping, separate from the employer plan covering the sponsor.
...
Visa issuance and renewal in Dubai require proof of active insurance linked to the applicant's Emirates ID file.
...
Each sponsored spouse, child, or parent needs an individual policy or a family rider that lists them by name on the DHA system.
...
Log into the DHA e-services portal or ask HR for the policy number, then confirm status is Active with end date after your visa renewal window.
...
Policy renewal: Most DHA-approved annual policies renew automatically if payment is received before the expiry date. Many insurers send renewal notices 60 to 30 days before expiry. Mark your renewal date and arrange payment at least 2 weeks before expiry to avoid any gap.
...
Retail premiums scale by age, network, and maternity add-ons; use the table below for planning, not quoting.
...
Minimums still mirror EBP benefits: emergency cover, GP access with capped co-pays, referral-based specialists, basic maternity, and formulary drugs, with pre-existing condition waiting caps on EBP products.
