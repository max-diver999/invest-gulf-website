...
At 20% down, a 10% price rise can produce roughly 50% return on equity before financing cost. The same leverage magnifies losses if prices fall.
...
Some buyers pay cash for a discount, then refinance after the bank seasoning period to redeploy capital.
...
Under 2026 guidance, a mortgaged property can qualify if the registered title value is AED 2M or above and the loan is from a UAE-licensed bank. Confirm with GDRFA at application time.
...
However, the previous rule (fully paid equity required) still appears in some official guidance. Verify with GDRFA/ICP at your transaction date, this is one area where the official position must be confirmed rather than assumed from secondary sources.

## What if Dubai prices fall after you take a mortgage?

Scenario 2: 30% market correction
- Property value: AED 2,000,000 to AED 1,400,000
